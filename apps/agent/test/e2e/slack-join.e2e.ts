...
async function main() {
	...
	if (!JOINS_FOR_REAL) {
		console.log(
			"NOTE  Gisul joins no channel in this run. A join is permanent.",
		);
	}
	...
	if (!publicUnjoined) {
		record(
			...
			true,
			"skipped: Gisul is already in every public channel",
		);
	...
	if (!privateUnjoined) {
		record(
			...
			true,
			"skipped: no private channel is visible to Gisul",
		);
	} else if (!JOINS_FOR_REAL) {
	...
	record(
		...
		canInvite,
		canInvite
			? "a user token lets Gisul add itself to a private channel"
			: "no user token, so Gisul cannot add itself to a private channel",
	);
