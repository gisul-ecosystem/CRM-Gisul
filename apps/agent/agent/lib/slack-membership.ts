...
function explain(error: string): string {
	switch (error) {
		case "no_user_grant":
			return "This workspace did not grant Gisul permission to add itself to a private channel.";
		case "channel_not_found":
			return "Slack cannot see this channel. A member has to invite Gisul.";
		case "is_archived":
			return "This channel is archived. Somebody has to unarchive it before Gisul can join.";
		case "missing_scope":
		...
		case "unknown_bot_user":
			return "Slack did not report which user Gisul is.";
		default:
...
export async function createSlackChannel(
	...
	if (!token) {
		return {
			error: isPrivate
				? "This workspace did not grant Gisul permission to create a private channel."
